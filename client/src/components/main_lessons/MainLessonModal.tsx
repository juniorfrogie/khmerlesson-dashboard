...
export default function MainLessonModal({
  isOpen,
  onClose,
  mainLesson,
}: MainLessonModalProps) {
  const [formData, setFormData] = useState<any>(null);
  const { toast } = useToast();

  const createMutation = useMutation({
    mutationFn: async (data: any) =>
      await apiRequest("POST", "/api/main-lessons", data),
    onSuccess: async () => {
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: any) =>
      await apiRequest("PATCH", `/api/main-lessons/${mainLesson?.id}`, data),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["main-lessons"] });
      await queryClient.invalidateQueries({
        queryKey: ["/api/dashboard/stats"],
