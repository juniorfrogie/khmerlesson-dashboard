...
export default function MainLessonViewDetailModal({
  mainLesson,
  isOpen,
  onClose,
}: MainLessonViewDetailModalProps) {
  ...
  const getLessons = async ({ queryKey }: any) => {
    ...
    const response = await apiRequest(
      "GET",
      `/api/main-lessons/${params}/lessons`,
    );
    return await response.json();
  };

  const { data: lessons = [], isFetching } = useQuery<Lesson[]>({
    queryKey: ["main-lessons-detail", mainLesson.id],
    queryFn: getLessons,
