...
export default function LessonTypeView({ onDelete }: LessonTypeViewProps) {
  ...
  const [selectedLessonTypeList, setSelectedLessonTypeList] = useState<
    number[]
  >([]);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingLessonType, setEditingLessonType] = useState<LessonType | null>(
    null,
  );
  const [lessonTypeViewDetail, setLessonTypeViewDetail] =
  ...
  const getAllLessonType = async ({ queryKey }: any) => {
    ...
    const response = await apiRequest(
      "GET",
      `/api/lesson-type?search=${params.search}&limit=${params.limit}&offset=${params.offset}`,
    );
    return await response.json();
  };

  const { data: pageData = { data: [], total: 0 }, isLoading } = useQuery<LessonTypePageData>({
    queryKey: [
      "lesson-type",
      {
        search: searchTerm,
  ...
  const lessonTypeTableHeaders = ["Icon", "Title", "Updated", "Action"];
  ...
  const toggleSelectAll = () => {
    if (selectedLessonTypeList.length === pageData.data.length) {
      setSelectedLessonTypeList([]);
    } else {
      setSelectedLessonTypeList(pageData.data.map((l) => l.id));
    }
  };
  ...
  const next = () => {
    ...
    setOffset(Math.min(min, pageData.total));
      ...
      <Card className="border border-gray-200">
        <CardContent className="p-6 space-y-6">
          ...
          <div className="border border-gray-200 rounded-lg overflow-hidden">
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-200">
                  {pageData.data.length === 0 ? (
                    ...
                    pageData.data.map((lessonType) => (
                      <tr key={lessonType.id} className="hover:bg-gray-50">
                        <td className="p-4">
                          <Checkbox
                            checked={selectedLessonTypeList.includes(
                              lessonType.id,
                            )}
                            onCheckedChange={() =>
                              toggleLessonTypeSelection(lessonType.id)
                            }
                          />
                        </td>
                        <td className="p-4">
            ...
            {/* {pageData.data.length > 0 && (
              <div className="p-6 border-t border-gray-200 flex items-center justify-between">
                <div className="text-sm neutral-medium">
                  Showing 1 to {pageData.data.length} of{" "}
                  {pageData.data.length} lesson type
                </div>
                <div className="flex items-center space-x-2">
                  ...
                  <Button
                    variant="outline"
                    size="sm"
                    className="bg-fluent-blue text-white"
                  >
                    1
                  </Button>
                  <Button variant="outline" size="sm" disabled>
            ...
            <Pagination
              title="lesson types"
              currentLength={pageData.data.length ?? 0}
              limit={limit}
              offset={offset}
