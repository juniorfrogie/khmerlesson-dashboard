...
export default function LessonTypeViewDetailModal({
  lessonType,
  isOpen,
  onClose,
}: LessonTypeViewDetailModalProps) {
  ...
  const { data: lessons = [], isFetching } = useQuery<Lesson[]>({
    queryKey: ["lesson-type-detail", lessonType.id],
    queryFn: getLessons,
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden">
        ...
        <div className="overflow-y-auto max-h-[calc(90vh-140px)] custom-scrollbar">
          {isFetching ? (
            <p className="neutral-light text-sm">Please wait...</p>
          ) : (
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex items-center space-x-4">
                          <div className="w-16 h-16 bg-blue-100 rounded-lg flex items-center justify-center">
                            {lessonType?.iconMode === "file" ? (
                              <img
                                src={`${lessonType?.iconUrl}`}
                                width="24"
                                height="24"
                                alt={lessonType?.title}
                              />
                            ) : (
                          ...
                          <div>
                            <CardTitle className="text-2xl neutral-dark">
                              {lesson.title}
                            </CardTitle>
                            <p className="neutral-medium mt-1">
                              {lesson.description}
                            </p>
                            <div className="flex items-center space-x-2 mt-3">
                              ...
                              >
                                ...
                                {lessonType &&
                                  lessonType.title?.charAt(0).toUpperCase() +
                                    lessonType.title?.slice(1)}
                              </Badge>
                              <Badge
                                className={
