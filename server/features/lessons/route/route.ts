...
router.get("/", async (req, res) => {
    try {
      ...
      if(req.query.limit && req.query.offset){
        ...
        lessons = await controller.getLessons(limit, offset)
        lessonCount = await controller.getLessonCount()
      }else{
        lessons = await controller.getAllLessons()
      }
  ...
  router.get("/:id", async (req, res) => {
    try {
      ...
      const lesson = await controller.getLesson(id);
  ...
  router.post("/", async (req, res) => {
    try {
      ...
      const lesson = await controller.createLesson(validatedData);
  ...
  router.patch("/:id", async (req, res) => {
    try {
      ...
      const lesson = await controller.updateLesson(id, validatedData);
  ...
  router.delete("/:id", async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const deleted = await controller.deleteLesson(id);
