...
router.get("/", async (req, res) => {
    try {
        ...
        if(req.query.limit && req.query.offset){
            ...
            mainLessons = await controller.getMainLessons(limit, offset)
            mainLessonCount = await controller.getMainLessonCount()
        }else{
            mainLessons = await controller.getAllMainLessons()
        }
...
router.get("/:id/lessons", async (req, res) => {
    try {
        ...
        const result = await controller.getAllLessonsByMainLesson(parseInt(id))
...
router.get("/:id",  async (req, res) => {
    try {
        const { id } = req.params
        const mainLesson = await controller.getMainLesson(parseInt(id))
        if(!mainLesson){
...
router.post("/", async (req, res) => {
    try {
        ...
        const mainLesson = await controller.createMainLesson(validatedData);
...
router.patch("/:id", async (req, res) => {
    try {
        ...
        const mainLesson = await controller.updateMainLesson(id, validatedData)
...
router.delete("/:id", async (req, res) => {{
    try {
        const id = parseInt(req.params.id)
        const deletedMainLesson = await controller.deleteMainLesson(id)
